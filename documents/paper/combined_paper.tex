...
Among feasible approaches, Simple Exponential Smoothing (SES) performs best (RMSE = 1.0598), followed closely by Holt--Winters. Both adaptive smoothing methods outperform Rolling and Global SARIMA. The Global SARIMA model exhibits the highest bias (0.3800) and a relatively large forecast error variance (1.1741), indicating that parameter contamination from pre-break observations substantially distorts forecasts. Rolling SARIMA reduces bias somewhat but remains less competitive than smoothing-based approaches.

\begin{table}[H]
\centering
\small
\caption{Mean Single Break: 300 simulations}
\label{tab:mean_single_results}
...
\centering
\small
\caption{Parameter Recurring (p=0.95): 300 simulations}
\label{tab:parameter_recurring_p0.95}
\begin{tabular}{lrrrr}
\toprule
Method & RMSE & MAE & Bias & Var(error) \\
\midrule
...
\centering
\small
\caption{Variance Single Break: 300 simulations}
\label{tab:variance_single_results}
\begin{tabular}{lrrrrrrr}
\toprule
Method & RMSE & MAE & Bias & Var(error) & Coverage80 & Coverage95 & LogScore \\
\midrule
GARCH & 2.0311 & 1.3335 & 0.2884 & 4.0423 & 0.0000 & 0.0000 & NaN \\
...
SARIMA Avg-Window & NaN & NaN & NaN & NaN & NaN & NaN & NaN \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
