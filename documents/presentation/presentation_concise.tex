...
\begin{frame}{Result 3.3: Variance Performance (Numerical)}
  ...
  \begin{table}
    ...
    \begin{tabular}{lcccc}
      \toprule
      \textbf{Break Magnitude} & \textbf{SARIMA Global} & \textbf{SARIMA Rolling} & \textbf{GARCH(1,1)} & \textbf{SARIMA Post-Break} \\
      \midrule
      $\sigma \times 1.5$ & 1.785 & 1.802 & 1.805 & 1.764 \\
      $\sigma \times 2.0$ & 2.392 & 2.415 & 2.414 & 2.351 \\
      $\sigma \times 3.0$ & 3.584 & 3.612 & 3.613 & 3.528 \\
      $\sigma \times 5.0$ & 6.182 & 6.241 & 6.219 & 6.104 \\
      \bottomrule
    \end{tabular}
    }
    \caption{Average Forecast RMSE across 200 Monte Carlo Replications ($T=400$)}
  \end{table}
  \vspace{-0.3cm}
  \begin{itemize}
    \item \textbf{Scale of Error:} Mean forecast RMSE scales approximately linearly with the innovation standard deviation post-break.
    \item \textbf{Global Stability:} In variance-only breaks, global SARIMA remains robust because the mean parameters ($\mu, \phi$) are stable over the full sample.
  \end{itemize}
\end{frame}
...
\begin{frame}{Selected References}
  \renewcommand{\bibsection}{} % Remove redundant 'References' header
  \vspace{0.1cm}
  {\tiny
