...
# =========================================================
# SECTION A (MAHIR): PARAMETER BREAK (kept as-is in logic)
# =========================================================
def simulate_param_break(T=400, Tb=200, phi1=0.3, phi2=0.9, sigma=1.0):
    y = np.zeros(T)
...
def forecast_rolling_ar(y, window=100):
    model = ARIMA(y[-window:], order=(1, 0, 0)).fit()
    return model.forecast(steps=1)[0]

def mc_simulate(n_sim=200, T=400, Tb=200, phi1=0.3, phi2=0.9):
    errors_global = []
    errors_rolling = []

    for _ in range(n_sim):
        y = simulate_param_break(T=T, Tb=Tb, phi1=phi1, phi2=phi2)

        true_next = y[-1]  # approx true (not perfect but fine for demonstration)

        fg = forecast_global_ar(y[:-1])
        fr = forecast_rolling_ar(y[:-1])

        errors_global.append((true_next - fg) ** 2)
        errors_rolling.append((true_next - fr) ** 2)

    rmse_global = np.sqrt(np.mean(errors_global))
    rmse_rolling = np.sqrt(np.mean(errors_rolling))
    return rmse_global, rmse_rolling


# =========================================================
# SECTION B (AADYA): VARIANCE BREAKS
# =========================================================
