...
\begin{center}
  \begin{tikzpicture}[
    scale=1,
    every node/.style={rectangle, rounded corners, minimum width=2.5cm, minimum height=0.8cm, text centered, font=\small}
  ]
  
  \node[fill=blue!20] (root) at (5, 8) {Is there a break?};
  
  \node[fill=green!20] (yes) at (2, 6.5) {Yes};
  \node[fill=red!20] (no) at (8, 6.5) {No (stationary)};
  
  \node[fill=yellow!20] (type) at (2, 5) {Break Type?};
  
  \node[fill=orange!20] (var) at (0, 3.5) {Variance};
  \node[fill=orange!20] (mean) at (2, 3.5) {Mean};
  \node[fill=orange!20] (param) at (4, 3.5) {Parameter};
  
  \node[fill=cyan!20] (var\_rec) at (0, 2) {GARCH or\,Rolling ARIMA};
  \node[fill=cyan!20] (mean\_rec) at (2, 2) {SARIMA Rolling\,or Break Dummy};
  \node[fill=cyan!20] (param\_rec) at (4, 2) {Markov-Switching\,or Rolling};
  
  \node[fill=green!20] (global) at (8, 2) {Use Global\,ARIMA};
...
\textbf{Practical Rule:} Test for breaks first (e.g., Chow, CUSUM). If detected, apply break-adaptive method matching break type.
\end{frame}
