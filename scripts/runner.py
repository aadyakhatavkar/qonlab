...
def run_variance_breaks():
    ...
    print(f"      ✓ Completed in {elapsed:.1f}s ({len(df_var_single)} rows)")
    all_dfs.append(df_var_single)
    
    # Recurring variance break with multiple persistence levels
    ...
    for p in PERSISTENCE_LEVELS:
        ...
        elapsed = time.time() - t0
        print(f"✓ ({elapsed:.1f}s, {len(df_var_recurring)} rows)")
...
def run_mean_breaks():
    ...
    elapsed = time.time() - t0
    print(f"      ✓ Completed in {elapsed:.1f}s ({len(df_mean_single)} rows)")
...
def run_parameter_breaks():
    ...
    elapsed = time.time() - t0
    print(f"      ✓ Completed in {elapsed:.1f}s ({len(df_param_single)} rows)")
