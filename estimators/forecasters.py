...
def forecast_sarima_global(y_train, horizon=1, order=(1, 0, 1), seasonal_order=(1, 0, 0, 12)):
    """Simple wrapper: fit SARIMA on full training series and return scalar 1-step forecast."""
    try:
        model = ARIMA(y_train, order=order, seasonal_order=seasonal_order, trend="n")
        res = model.fit()
        fc = res.forecast(steps=horizon)
        return float(fc[0]) if len(fc) > 0 else np.nan
    except Exception:
        return np.nan


def forecast_sarima_rolling(y_train, window=80, horizon=1, order=(1, 0, 1), seasonal_order=(1, 0, 0, 12)):
    """Simple wrapper: fit SARIMA on rolling window and return scalar 1-step forecast."""
    try:
        y_win = y_train[-window:] if window < len(y_train) else y_train
        model = ARIMA(y_win, order=order, seasonal_order=seasonal_order, trend="n")
        res = model.fit()
        fc = res.forecast(steps=horizon)
        return float(fc[0]) if len(fc) > 0 else np.nan
    except Exception:
...
def forecast_markov_switching_ar(y_train, k_regimes=2):
    """Fit MarkovRegression with lag exog and return 1-step scalar forecast (phi-only style)."""
    try:
        ...
        # Extract coefficients for each regime (x1[i])
        coefs = [params.get(f"x1[{i}]", np.nan) for i in range(k_regimes)]
        if not np.all(np.isfinite(coefs)):
            return np.nan

        forecast = sum(probs[i] * coefs[i] * y[-1] for i in range(k_regimes))
